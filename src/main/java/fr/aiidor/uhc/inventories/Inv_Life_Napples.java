...
public class Inv_Life_Napples extends GuiBuilder {
	...
	public HashMap<String, ItemStack> getDictionary() {
		...
		if (UHC.getInstance().getGameManager().hasGame()) {
			...
			dictionary.put("A", new ItemBuilder(Material.GOLD_CHESTPLATE, Lang.INV_LIFE_ABSO.get().replace(LangTag.VALUE.toString(), game.getSettings().napple_abso + " §c❤")).setLore(Arrays.asList(
	...
	public void onClick(GuiClickEvent event) {
		...
		if (e.getSlot() == 2) {
			...
			if (game.getSettings().napple_abso >= 10) game.getSettings().napple_abso = 10;
